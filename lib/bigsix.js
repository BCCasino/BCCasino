...
Class('BigSixWheel',{
	has: {
		...
		Bets: {
			is: "ro",
			init: {"1":1,"2":2,"5":5,"10":10,"20":20,"flag":40,"joker":40}
		},
		PlayedBets: {
			is: "rw",
			init: []
	...
	methods: {
		...
		PlaceBet: function(secret,bet,amount,callback) {
			exports.MaxBet(function(max) {
				if(amount < max)
				{
					client.getBalance(secret,function(err,bal) {
						...
						{
							setPlayedBets(getPlayedBets().push([secret, bet,amount]));
							callback(true);
						}
						else
							callback(false);
		...
		PayBets: function() {
			...
			for(var x =0;x<getPlayedBets().length;x++)
			{
				...
				{//winner, winner, chicken dinner
					...
					this.getClient().move("",bet[0],amountWon,function(err) { });
				}
				else
				{//try again next time
					this.getClient().move(bet[0],"",bet[2],function(err) {});
				}
			}
		},
		Seed: function() {
