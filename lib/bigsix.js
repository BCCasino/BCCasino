			...
			this.maxBet(function(max) {
				...
				if(amount < max && amount >= 0)
				{
					Account.balance(function(bal) {
						...
						{
							var found =false 
							var playersBets = self.getPlayedBets();
							...
							found=false;
							console.log("about to check existing bets to increment or create new: "+sys.inspect(playersBets));
							for(var x in playersBets[0])
							{
								if(playersBets[x][0]==Account && playersBets[x][1]==bet)
								{
									playersBets[x][2]+=parseFloat(amount);
									found=true;
									break;
								}
