...
Class('BigSixWheel',{
	...
	methods: {
		...
		getPlayersBets: function(Account) {
			...
			for(var x in b)
			{
				if(b[x][0].getSecret()==Account.getSecret())
				{
					z.push([b[x][1],b[x][2]]);
					amt+=b[x][2];
				}
		...
		removeBet: function(Account,bet,amount,callback) {
			...
			if (amount >= 0) {
				...
				for(var x in b)
				{
					if(b[x][0].getSecret()==Account.getSecret() && b[x][1]==bet)
					{
...
=======
							for(var x in bets)
>>>>>>> e87dccdd15140da0417f1268d05b51697b364977
							{
								if(bets[x][0].getSecret()==Account.getSecret() && bets[x][1]==bet)
								{
