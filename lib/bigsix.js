...
Class('BigSixWheel',{
	...
	methods: {
		...
		removeBet: function(Account,bet,amount,callback) {
			...
			if(!found)
			{
				Account.log("Caught hacking, trying to remove invalid bet type!");
			...
			Account.log("Removed bet on "+bet+" for "+amount+" was successful: "+found);
		...
		placeBet: function(Account,bet,amount,callback) {
			...
			this.maxBet(function(max) {
				...
				Account.log("Placing bet on "+bet+" for "+amount);
				if(amount < max && amount >= 0)
				{
					Account.balance(function(bal) {
						...
						if(amount <= bal) //player has enough money
						{
							...
							if(bets[1]+amount >bal)
							{	//Prevent double spending...				
								Account.getSocket().emit('exception',"Insufficient funds");
								Account.log("Placing bet on "+bet+" for "+amount + ", Insufficient Funds");
							...
							if(!found)
							{
								Account.log("Caught hacking, trying to place a bet on invalid tile");
							...
							Account.log("Placing bet on "+bet+" for "+amount + ", done");
						}
						else
						{
							Account.log("Placing bet on "+bet+" for "+amount + ", Insufficient Funds");
