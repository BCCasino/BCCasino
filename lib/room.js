...
function Room(io,channel,incallback,outcallback){
	var self = this;
	console.log("LOGIN: "+incallback);
	console.log("LOGOUT: "+outcallback);
	this.login=incallback;
	this.logout=outcallback;
	this.spun=true;
	...
	this.comm = io.of(channel).on('connection',function(socket){
		...
		socket.on('join',function(msg){
			...
			if(msg.secret === undefined || msg.secret.length != 64)
				acct=  new Account({socket:socket,BS:self.bigSix,log:self.log,room:self});
			else
				acct= new Account({socket:socket,BS:self.bigSix,secret: msg.secret,log:self.log,room:self});
			var ret = self.login(acct.getSecret());
			if(!ret)
			{
				socket.emit("exception","You are already online, please close any existing connections first!");
				socket.disconnect();
				return;
			}
...
try{
	var p = this.findPlayer(socket);	
	this.logout(p.account.getSecret());
