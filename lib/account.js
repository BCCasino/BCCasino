	...
	methods: {
		...
		withdraw: function(address,amount){
			...
			this.getWithdrawableBalance(function(bal){			
				this.getLog().log(this.getSecret(),"Withdrawing: "+amount+" of "+bal);
				if(amount<bal)//only what they are allowed to withdraw
					bal=amount;
				bal=parseFloat(bal);				
		...
		Log: function(msg)//used in bigsix
		{
			this.getLog().log(self.getSecret(),msg);
		}
	},
	after: {
