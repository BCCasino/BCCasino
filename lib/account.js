...
Class('Account',{
	...
	methods: {		
		...
		withdraw: function(address,amount){
			...
			{
				this.getSocket().emit("exception","Minimum withdraw of 0.01 BTC");
				return;
			}
			var self=this; 
			this.getLog().log(this.getSecret(),"Requested withdraw");
			this.getRoom().bigSix.removeAllBets(self); //REMOVE ALL BETS!
			this.getSocket().emit("removeBet",{ok:sucess,bets:[this.getRoom().bigSix.getPlayersBets(this)[0]]});//update client to have no bets
			this.getWithdrawableBalance(function(bal){			
				...
				self.getClient().sendFrom(self.getSecret(),address,amount,0,'','TheBitcoinWheel',function(err){
					if (err) {
						self.getSocket().emit("exception","Error withdrawing: "+err);
						console.log("EXCEPTION DURING WITHDRAW: "+err);
						return;
					}
					self.getWithdrawableBalance(function(balnow){
						if((bal-amount)-(balnow) >0)
						{
							console.log("Crediting transaction fee of "+(bal-amount)-(balnow));
							self.credit((bal-amount)-(balnow));
						}
						self.balance(function(finbal){
							self.getSocket().emit('Balance',finbal);
						});
						self.getSocket().emit('withdraw',"Withdraw succeeded");
					},0);
				});
			});
		},
